...
#' @param legend Logical: If TRUE, show legends on plot, if \code{x} has column 
#' names
#' @param lwd Float: Line width. Default = 1
#' @param pty String: "m" to fill available device space, "s" for square plot. 
#' Default = "m"
...
mplot <- function(x,
    ...
    .ncol <- NCOL(x)
    if (is.null(mar)) {
        mar <- if (.ncol == 1) c(1.2, 1.2, .4, .4) else c(0, 0, 0, 0)
    }
    if (is.null(oma)) {
        if (.ncol == 1) {
    ...
    par(
        bg = bg, pty = pty, mar = mar,
        mfrow = c(.ncol, 1), oma = oma, new = new
    )
    .names <- colnames(x)
    ...
    for (i in seq(.ncol)) {
        ...
        if (i == 1) {
            if (!is.null(main)) {
                mtext(main,
                    col = col.main, line = 0.3,
                    adj = 0, font = 2, xpd = TRUE
                )
            }
        }
        if (legend & !is.null(.names)) {
            mtext(.names[i], col = col.legend, line = -1.5, adj = .99)
        }
    }
} # music::mplot
