...
freq2wave <- function(
  ...
  wave <- sapply(seq(n.freqs), function(i) {
    switch(
      ...
      triangle = 2 *
        abs(2 * (.t * frequency[i] - floor(.5 + .t * frequency[i]))) -
        1
    )
  })
  ...
  if (attack.time > 0) {
    ...
    n.attack.samples <- round(min(
      n.samples,
      attack.time / 1000 * sample.rate
    ))
    attack[seq(n.attack.samples)] <- seq(
      0,
      1,
      length.out = n.attack.samples
    )
    wave <- wave * attack
  }
  ...
  if (inner.release.time > 0) {
    ...
    n.inner.release.samples <- round(min(
      n.samples,
      inner.release.time / 1000 * sample.rate
    ))
    inner.release[n.samples - (seq(n.inner.release.samples) - 1)] <- seq(
      0,
      1,
      length.out = n.inner.release.samples
    )
    wave <- wave * inner.release
  }

  .names <- if (!is.null(names(frequency))) {
    names(frequency)
  } else {
    paste(frequency, "Hz")
  }
